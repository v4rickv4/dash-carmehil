 ...
 *
 * Supports Meta Ads (dados_meta_ads_carmehil) and Google Ads (dados_google_ads).
 */
...
export function computeInsights(campaignGroups, totals) {
  ...
  if (byLeads[0] && byLeads[0].leads > 0) {
    insights.push({
      ...
      description: `"${byLeads[0].campanha}" gerou ${Math.round(byLeads[0].leads)} conversões/leads no período.`,
  ...
  if (withLeads.length > 0) {
    ...
    insights.push({
      ...
      type: 'positive',
      icon: 'TrendingDown',
      title: 'Menor custo por conversão',
      description: `"${byCpl[0].campanha}" apresentou Custo/Conversão de R$ ${byCpl[0].cpl.toFixed(2).replace('.', ',')}.`,
      metric: byCpl[0].cpl,
      metricLabel: 'Custo/Conversão',
    });
  }
